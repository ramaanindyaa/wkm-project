                        ...
                        <div class="grid grid-cols-2 gap-6">
                            <div class="flex items-center justify-between rounded-2xl border border-[#E6E7EB] p-5 gap-4 bg-white">
                                <div class="flex flex-col gap-2">
                                    ...
                                    <p class="font-semibold text-lg leading-[27px]">{{ \Carbon\Carbon::parse($workshop->time_at)->format('H:i A') }} - Finish</p>
                                </div>
                            ...
                            <div class="flex items-center justify-between rounded-2xl border border-[#E6E7EB] p-5 gap-4 bg-white">
                                <div class="flex flex-col gap-2">
                                    ...
                                    <p class="font-semibold text-lg leading-[27px]">
                                        {{ \Carbon\Carbon::parse($workshop->started_at)->translatedFormat(' d F Y') }}
                                    </p>
                                </div>
                    ...
                    <div class="relative flex w-full h-[360px] rounded-2xl overflow-hidden bg-[#D9D9D9]">
                        <div class="absolute left-5 top-1/2 transform -translate-y-1/2 flex flex-col w-[260px] h-fit max-h-[320px] gap-4 rounded-2xl p-5 bg-white">
                            <div class="flex w-full h-[124px] rounded-xl overflow-hidden">
                                <img src="{{asset(Storage::url($workshop->venue_thumbnail))}}" class="w-full h-full object-cover" alt="location">
                            </div>
                            <div class="flex flex-col gap-3 justify-between">
                                <p class="font-medium leading-[25.6px] text-aktiv-grey line-clamp-4">
                                    {{ $workshop->address }}
                                </p>
                                <a href="http://maps.google.com/?q={{ urlencode($workshop->address) }}" class="font-semibold text-aktiv-orange">View in Google Maps</a>
                            </div>
                        </div>
                        <img src="{{asset(Storage::url($workshop->bg_map))}}" class="w-full h-full object-cover" alt="maps">
                    </div>
                </div>
            </section>
            <section id="Sidebar" class="flex flex-col w-[420px] gap-8">
                ...
                <div class="flex flex-col rounded-3xl pt-8 gap-6 bg-white">
                    ...
                    <div class="flex flex-col">
                        @if ($workshop->is_open && !$workshop->has_started)
                        ...
                        @else
                        <a class="flex items-center justify-center mx-8 h-16 rounded-xl px-6 gap-[10px] bg-[#E6E7EB] font-semibold text-lg leading-[27px] text-white mb-8">Join Workshop</a>
                            <div class="p-4 bg-aktiv-red">
                                <p class="font-semibold text-lg leading-[27px] text-center text-white">Oops! Sorry, it seems that registration for this workshop is now closed 🙌🏻 . </p>
                            </div>
                        @endif
                    </div>
                </div>
